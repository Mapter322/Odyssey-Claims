...
public final class AdminClaimTargets {

    private AdminClaimTargets() {
    }

    public static void registerAll(MinecraftServer server) {
        CadmusSaveData.getAllConditions(server).forEach(AdminClaimTargets::register);
    }
    ...
    public static void unregister(String condition) {
        SettingDefinitions.unregister(SettingScope.ADMIN_CLAIM, condition);
    }
}
