...
public final class AdminClaimDefaultsConfig {
    ...
    private static Path configPath() {
        return CadmusConfig.configFolder().resolve(Cadmus.MOD_ID).resolve("adminclaimsdefault.json");
    }
    ...
    private static Map<String, Boolean> parse(String text) {
        ...
        if (element == null || !element.isJsonObject()) {
            LOGGER.warn("Admin claim defaults have no 'settings' object, using built-in defaults");
            return generate();
        }
    ...
    private static Boolean parseState(String key, JsonElement element) {
        ...
        return switch (element.getAsString().toLowerCase(Locale.ROOT)) {
            ...
            default -> {
                LOGGER.warn("Unknown admin claim default value '{}' for '{}', skipping", element.getAsString(), key);
                yield null;
            }
        };
    }
